...
class Waypoints(object):
    ...
    def remove_completed(self, location, ego_transform=None):
        ...
        if len(self.waypoints) == 1:
            from os import path
            if not path.exists("/home/erdos/workspace/results/finished.txt"):
                final_results = open("/home/erdos/workspace/results/finished.txt", "x")
            #self.waypoints.popleft();


        if self.waypoints is None or len(self.waypoints) == 0 or len(self.waypoints) == 1:
