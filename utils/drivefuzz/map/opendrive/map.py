...
class Map:
  ...
  def load(self, filename):
    ...
    root = tree.getroot()
    if root is  None:
      raise Exception("Map XML failed!")

    if root.tag != 'OpenDRIVE':
      raise Exception("the tag is not OpenDRIVE")

    # 1. header
    raw_header = root.find('header')
    if raw_header is  None:
      raise Exception("Open drive map missing header")
    ...
    if raw_roads is None:
      raise Exception("Open drive map missing roads")
