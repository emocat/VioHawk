...
class CyberBridgeInstance:
    ...
    def calculate_distToDEstination(self):
        ...
        ego_polygonPointList = self.get_four_polygon_point_list_of_ego(original_point, heading_of_ego, lengthen_of_ego,
                                                                       width_of_ego)
    ...
    def parse_message_of_pose(self, pose):
        ...
        if self.Canbus != {} and self.truth != {}:
            ...
            width_of_ego = self.Ego["size"]["width"]
            ego_polygonPointList = []
            ego_polygonPointList = self.get_four_polygon_point_list_of_ego(original_point, heading_of_ego,
                                                                           lengthen_of_ego, width_of_ego)
            self.ego_position_area = Polygon(ego_polygonPointList)

            head_middle_point = self.get_the_head_middle_point_of_ego(original_point, heading_of_ego, lengthen_of_ego,
                                                                      width_of_ego)
            back_middle_point = self.get_the_back_middle_point_of_ego(original_point, heading_of_ego, lengthen_of_ego,
                                                                      width_of_ego)

            currentLane = self.check_current_lane()  # the currentLane API
            self.Ego["currentLane"] = currentLane
            ahead_area_polygon = self.calculate_area_of_ahead(head_middle_point, self.Ego["pose"]["heading"],
                                                              width_of_ego)
            back_area_polygon = self.calculate_area_of_ahead(back_middle_point, self.Ego["pose"]["heading"],
                                                             width_of_ego)
            back_area_polygon2 = self.calculate_area_of_ahead2(back_middle_point, self.Ego["pose"]["heading"])

            ahead_area_polygon_for_opposite = self.calculate_area_of_ahead(head_middle_point,
                                                                           self.Ego["pose"]["heading"], width_of_ego,
                                                                           dist=30)
            ...
            backward_area_left = self.calculate_area_of_back_left(back_middle_point, self.Ego["pose"]["heading"],
                                                                  width_of_ego)
            backward_area_right = self.calculate_area_of_back_right(back_middle_point, self.Ego["pose"]["heading"],
                                                                    width_of_ego)
            ...
            stopSignAhead = self.calculate_distance_to_stopline_of_sign_ahead(
                ahead_area_polygon)  # the stopSignAhead API
            ...
            if distance < 2:
                ...
                if self.position_for_check == {}:
                    ...
                    self.position_check_num = 0
                elif abs(self.position_for_check["x"] - self.Ego["pose"]["position"]["x"]) <= threshold and abs(
                        self.position_for_check["y"] - self.Ego["pose"]["position"]["y"]) <= threshold and abs(
                        self.position_for_check["z"] - self.Ego["pose"]["position"]["z"]) <= threshold:
            ...
            self.Find_Priority_NPCs_and_Peds(back_area_polygon2, left_area_polygon, right_area_polygon,
                                             backward_area_left, backward_area_right)
    ...
    def find_npc_opposite(self, oblist, ahead_area_polygon):
        ...
        for _i in oblist:
            ...
            if ahead_area_polygon.distance(the_area) == 0 and _i["type"] == 5:
                ...
                if abs(heading_of_npc - heading_of_ego) > 3 * math.pi / 4 and abs(
                        heading_of_npc - heading_of_ego) < 5 * math.pi / 4:
                    _temp[_i["name"]] = ego.distance(the_area)
                    return_value = _i["name"]
        for key in _temp:
            if _temp[key] < _temp[return_value]:
    ...
    def _sub_Find_Priority_NPCs_and_Peds(self, ahead_square_area, left_area_polygon, right_area_polygon, num_of_track,
                                         backward_area_left, backward_area_right):
        previous_trace = self.trace[num_of_track]
        ...
        for obstacle in obstacles:
            ...
            if obstacle["type"] == 5:
                ...
                if previous_trace["ego"]["planning_of_turn"] != 0 and previous_trace["ego"]["isLaneChanging"] == False:
                    points = []
                    if abs(heading_of_npc - heading_of_ego) > math.pi / 4 and abs(
                            heading_of_npc - heading_of_ego) < 3 * math.pi / 4:
                ...
                if previous_trace["ego"]["isLaneChanging"] == True:
                    if abs(heading_of_npc - heading_of_ego) < math.pi / 4:
                        ...
                        if previous_trace["ego"]["planning_of_turn"] == 1 and backward_area_left.distance(
                                the_area) == 0:  # left lane changing
                            if obstacle["distToEgo"] < 10 and obstacle["speed"] > ego_speed:
                                previous_trace["ego"]["PriorityNPCAhead"] = True
                                print("Find priority NPC for left lane changing!")
                        if previous_trace["ego"]["planning_of_turn"] == 2 and backward_area_right.distance(
                                the_area) == 0:  # right lane changing
                            if obstacle["distToEgo"] < 10 and obstacle["speed"] > ego_speed:
                                previous_trace["ego"]["PriorityNPCAhead"] = True
    ...
    def Find_Priority_NPCs_and_Peds(self, ahead_square_area, left_area_polygon, right_area_polygon, backward_area_left,
                                    backward_area_right):
        num_of_track = -20
        if len(self.trace) >= -num_of_track:
            self._sub_Find_Priority_NPCs_and_Peds(ahead_square_area, left_area_polygon, right_area_polygon,
                                                  num_of_track, backward_area_left, backward_area_right)
        ...
        if self.reach_destinaton or self.Check_The_vehicle_is_stuck_or_not == "Stuck!":
            if len(self.trace) >= -num_of_track:
                for num in range(num_of_track + 1, 0):
                    self._sub_Find_Priority_NPCs_and_Peds(ahead_square_area, left_area_polygon, right_area_polygon, num,
                                                          backward_area_left, backward_area_right)
