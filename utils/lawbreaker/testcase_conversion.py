...
def convert_to_lawbreaker_testcase(scenario: dataparser.scenario.Scenario, traces: list, map_info):
    result = {}
    agent_names = []

    result["ScenarioName"] = "mutation"
    result["MapVariable"] = ""
    ...
    for t in range(trace_time):
        ...
        for trace_data_npc in trace_data["NPCs"]:
            ...
            poly_list = lawbreaker_helper.get_four_polygon_point_list_of_ego(
                [trace_data_npc["Position"]["x"], trace_data_npc["Position"]["z"]], single_trace_truth_npc["theta"], trace_data_npc["Scale"]["y"], trace_data_npc["Scale"]["x"]
            )
