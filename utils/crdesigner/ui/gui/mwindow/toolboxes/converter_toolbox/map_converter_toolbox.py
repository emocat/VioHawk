...
class RequestRunnable(QRunnable):
    ...
    def run(self):
        ...
        QMetaObject.invokeMethod(
            self.mapConversionToolbox, "stopSpinner", Qt.QueuedConnection, Q_ARG(str, "Conversion Ended")
        )
...
class MapConversionToolbox(QDockWidget):
    ...
    def convert_osm_to_cr(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, "Warning", "Map unreadable: " + str(e), QMessageBox.Ok)
    ...
    def download_osm_map(self) -> Optional[str]:
        ...
        if not self.verify_osm_coordinate_input():
            QMessageBox.critical(self, "Warning", "cannot download, coordinates invalid", QMessageBox.Ok)
            return None
        else:
            download_around_map(
                name,
                float(self.converter_toolbox.osm_conversion_coordinate_latitude.text()),
