...
class RequestRunnable(QRunnable):
    ...
    def run(self):
        ...
        QMetaObject.invokeMethod(
            self.roadNetworkToolbox, "stopSpinner", Qt.QueuedConnection, Q_ARG(str, "Conversion Ended")
        )
...
class RoadNetworkToolbox(
    ...
    def __init__(
        self, current_scenario: Scenario, text_browser, callback, tmp_folder: str, selection_changed_callback, mwindow
    ):
    ...
    def update_lanelet(self):
        ...
        self.add_updated_lanelet(
            selected_lanelet.lanelet_id, selected_lanelet.left_vertices, selected_lanelet.right_vertices
        )
    ...
    def add_four_way_intersection(self):
        ...
        country_signs = globals()[
            "TrafficSignID"
            + SupportedTrafficSignCountry(self.current_scenario.scenario_id.country_id).name.capitalize()
        ]
    ...
    def connect_lanelets(self):
        ...
        connected_lanelet = MapCreator.connect_lanelets(
            selected_lanelet_one, selected_lanelet_two, self.current_scenario.generate_object_id()
        )
    ...
    def merge_with_successor(self):
        ...
        for suc in selected_lanelet_one.successor:
            new_lanelet = Lanelet.merge_lanelets(
                selected_lanelet_one, self.current_scenario.lanelet_network.find_lanelet_by_id(suc)
            )
    ...
    def fit_intersection(self):
        ...
        if (
            ...
            MapCreator.fit_intersection_to_predecessor(
                lanelet_predecessor, lanelet_successor, intersection, self.current_scenario.lanelet_network
            )
            self.callback(self.current_scenario)
