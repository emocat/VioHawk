...
class DynamicCanvas(FigureCanvas):
    ...
    def _select_lanelet(self, release: bool = False, lane_ids: list = None):
        ...
        if (
            len(self.animated_viewer.current_scenario.lanelet_network.lanelets) - self.num_lanelets != 0
        ...
        selected_obstacles = [
            obs
            for obs in self.animated_viewer.current_scenario.obstacles
        ...
        if not self.draw_lanelet_first_point:
            ...
            try:
                if self.add_to_selected:
                    ...
                    created_lanelet = MapCreator.create_straight(
                        3.0, lanelet_length, num_vertices, self.scenario.generate_object_id(), lanelet_type
                    )
            ...
            if self.add_to_selected:
                ...
                created_lanelet = MapCreator.connect_lanelets(
                    self.add_to_selected, created_lanelet, self.scenario.generate_object_id()
                )
    ...
    def activate_aerial_image(
        self, bing: bool, lat1: float, lon1: float, lat2: float, lon2: float, center_at_zero: bool
    ):
        """
        ...
        if bing:
            self.current_aerial_image, extent = get_aerial_image_bing(
                self.aerial_map_bounds[0],
                self.aerial_map_bounds[1],
                self.aerial_map_bounds[2],
                self.aerial_map_bounds[3],
            )
        else:
            self.current_aerial_image = get_aerial_image_ldbv(
                self.aerial_map_bounds[0],
                self.aerial_map_bounds[1],
                self.aerial_map_bounds[2],
                self.aerial_map_bounds[3],
            )
        self.image_limits = self.get_aerial_image_limits(
            self.aerial_map_bounds[0],
            self.aerial_map_bounds[1],
            self.aerial_map_bounds[2],
            self.aerial_map_bounds[3],
    ...
    def get_aerial_image_limits(
        self, lat1: float, lon1: float, lat2: float, lon2: float, center_at_zero: bool
    ) -> List[float]:
        """
