...
class GUISettings:
    ...
    def save_to_config(self):
        ...
        if config.BING_MAPS_KEY != "":
            ...
            if not check:
                ...
                warning_dialog.warning(
                    None, "Warning", "Specified Bing Maps key is wrong.", QMessageBox.Ok, QMessageBox.Ok
                )
        ...
        if config.LDBV_USERNAME != "" or config.LDBV_PASSWORD != "":
            ...
            if not check:
                ...
                warning_dialog = QMessageBox()
                warning_dialog.warning(
                    None, "Warning", "Specified LDBV username or password is wrong.", QMessageBox.Ok, QMessageBox.Ok
                )
                warning_dialog.close()
    ...
    def set_draw_params_gui(self):
        set_draw_params(
            trajectory=config.DRAW_TRAJECTORY,
    ...
    def apply_close(self) -> None:
        ...
        if self.has_valid_entries():
            ...
            set_draw_params(
                trajectory=self.window.chk_draw_trajectory.isChecked(),
