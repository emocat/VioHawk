...
def _open_scenario(mwindow, new_scenario, filename="new_scenario", pps=None):
    ...
    if SUMO_AVAILABLE:
        mwindow.animated_viewer_wrapper.cr_viewer.open_scenario(
            new_scenario, mwindow.obstacle_toolbox.sumo_simulation.config, planning_problem_set=pps, new_file_added=True
        )
        mwindow.obstacle_toolbox.sumo_simulation.scenario = mwindow.animated_viewer_wrapper.cr_viewer.current_scenario
    else:
        mwindow.animated_viewer_wrapper.cr_viewer.open_scenario(
            new_scenario, planning_problem_set=pps, new_file_added=True
        )
...
def file_save(mwindow):
    ...
    mwindow.scenario_saving_dialog.show(
        mwindow.animated_viewer_wrapper.cr_viewer.current_scenario,
...
def close_window(mwindow):
    ...
    reply = QMessageBox.warning(
        mwindow, "Warning", "Do you really want to quit?", QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes
    )
