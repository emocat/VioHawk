...
class CR2LaneletConverter:
    ...
    def __call__(self, scenario):
        ...
        self.right_ways = {}
        if (
            scenario.location is not None
            and not isinstance(scenario.location.gps_longitude, str)
            and abs(scenario.location.gps_longitude) <= 180
            and abs(scenario.location.gps_latitude) <= 90
        ):
            self.origin_utm = self.proj(scenario.location.gps_longitude, scenario.location.gps_latitude)
        elif (
            scenario.location is not None
            and isinstance(scenario.location.gps_longitude, str)
            and abs(float(scenario.location.gps_longitude)) <= 180
    ...
    def _convert_traffic_light(self, light: TrafficLight):
        ...
        x2, y2 = self.proj(
            self.origin_utm[0] + light.position[0] + 0.1, self.origin_utm[1] + light.position[1] + 0.1, inverse=True
        )
        x3, y3 = self.proj(
            self.origin_utm[0] + light.position[0] - 0.1, self.origin_utm[1] + light.position[1] - 0.1, inverse=True
        )
    ...
    def _create_stop_line_to_way_dictionary(self) -> Dict[int, str]:
        ...
        for ll in self.lanelet_network.lanelets:
            if ll.stop_line:
                ...
                x_start, y_start = self.proj(
                    self.origin_utm[0] + stop_line_start[0], self.origin_utm[1] + stop_line_start[1], inverse=True
                )
                x_end, y_end = self.proj(
                    self.origin_utm[0] + stop_line_end[0], self.origin_utm[1] + stop_line_end[1], inverse=True
                )
    ...
    def _convert_traffic_sign(self, sign: TrafficSign):
        ...
        x2, y2 = self.proj(
            self.origin_utm[0] + sign.position[0] + 0.25, self.origin_utm[1] + sign.position[1] + 0.25, inverse=True
        )
        id2 = self.id_count
