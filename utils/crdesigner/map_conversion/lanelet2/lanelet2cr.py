...
from commonroad.scenario.traffic_sign import (
    TrafficSignElement,
    TrafficSignIDGermany,  # type: ignore
...
class Lanelet2CRConverter:
    ...
    def __call__(
        ...
        scenario_id = ScenarioID(
            country_id=config.DEFAULT_SCENARIO_COUNTRY_ID,
            map_name=config.DEFAULT_SCENARIO_MAP_NAME,
            map_id=config.DEFAULT_SCENARIO_MAP_ID,
        )
        scenario = Scenario(
            dt=config.GLOBAL_TIMESTEP,
    ...
    def _traffic_sign_ways_to_traffic_signs(
        self, traffic_sign_ways: List, new_lanelet_ids: Dict[str, int]
