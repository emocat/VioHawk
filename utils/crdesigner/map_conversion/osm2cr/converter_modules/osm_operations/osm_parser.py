...
def get_graph_traffic_signs(
    nodes: Dict[int, rg.GraphNode], roads: Dict[int, rg.GraphEdge], traffic_signs: List[Dict]
...
def get_graph_nodes(
    ...
    for road in roads:
        ...
        for point_id in (int(road.attrib["from"]), int(road.attrib["to"])):
            ...
            if point_id not in nodes:
                nodes[point_id] = rg.GraphNode(point_id, current_point.x, current_point.y, OrderedSet())
    ...
    for traffic_sign in traffic_signs:
        ...
        if int(point_id) not in nodes:
            ...
            nodes[int(point_id)] = rg.GraphNode(int(point_id), current_point.x, current_point.y, OrderedSet())
            ...
            if config.SPLIT_AT_CORNER and is_corner(index, waypoints):
                try:
                    ...
                    if waypoint_id not in nodes:
                        ...
                        nodes[waypoint_id] = rg.GraphNode(id_int, waypoint.x, waypoint.y, OrderedSet())
                    edge_nodes.append((nodes[waypoint_id], index))
                except ValueError:
    ...
    for point_id, point in main_cross_points.items():
        if point_id in sub_cross_points and not close_to_intersection(point_id, comb_graph, main_graph):
            if point_id in main_node_dict:
                ...
                crossing_node = rg.GraphNode(int(point_id), point.x, point.y, OrderedSet())
...
def create_graph(file_path: str) -> rg.Graph:
    ...
    if config.EXTRACT_SUBLAYER:
        ...
        # get crossing nodes
        main_g, main_crossing_points = _create_graph(file_path, config.ACCEPTED_HIGHWAYS_MAINLAYER, combined_g.bounds)
        logging.info("extract sub layer")
        sub_g, sub_crossing_points = _create_graph(file_path, config.ACCEPTED_HIGHWAYS_SUBLAYER, combined_g.bounds)
        ...
        extended_main_graph, _ = _create_graph(
            file_path, config.ACCEPTED_HIGHWAYS_MAINLAYER, combined_g.bounds, new_crossing_nodes
        )
